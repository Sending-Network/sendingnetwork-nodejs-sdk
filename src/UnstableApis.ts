...
export class UnstableApis {
    ...
    public async getRoomAliases(roomId: string): Promise<string[]> {
        const r = await this.client.doRequest("GET", "/_api/client/unstable/org.sdn.msc2432/rooms/" + encodeURIComponent(roomId) + "/aliases");
        return r['aliases'] || [];
    }
     ...
     * @deprecated Please use the function of the same name in SDNClient. This will be removed in a future release.
     */
    public async getRelationsForEvent(roomId: string, eventId: string, relationType?: string, eventType?: string): Promise<{ chunk: any[] }> {
        let url = `/_api/client/unstable/rooms/${encodeURIComponent(roomId)}/relations/${encodeURIComponent(eventId)}`;
        if (relationType) {
            url += `/${relationType}`;
        }
        if (eventType) {
    ...
    public async getMediaInfo(mxcUrl: string): Promise<MSC2380MediaInfo> {
        ...
        return this.client.doRequest("GET", `/_api/media/unstable/info/${encodeURIComponent(domain)}/${encodeURIComponent(mediaId)}`);
    }
}
