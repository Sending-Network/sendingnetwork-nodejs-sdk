...
export interface RoomCreateOptions {
     ...
     * The alias will belong on the same node which created the room.
     */
    room_alias_name?: string;

    /**
     * The room version to set for the room.
     * If not provided, the node is to use its configured default.
     * If provided, the node will return a `400` error with the errcode `M_UNSUPPORTED_ROOM_VERSION` if it does not support the room version.
     */
    room_version?: string;
