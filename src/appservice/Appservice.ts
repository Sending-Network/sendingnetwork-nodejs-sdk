...
import { SDNBridge } from "./SDNBridge";
...
export interface IAppserviceRegistration {
    /**
     * Optional ID for the appplication service. Used by nodes to track which application
    ...
    /**
     * The token the application service uses to communicate with the node.
     */
    as_token: string;

    /**
     * The token the node uses to communicate with the application service.
     */
    hs_token: string;
    ...
    namespaces: {
        ...
        users: {
             ...
             * means that no other user on the node may register users that match this namespace.
             */
            exclusive: boolean;

            /**
             * The regular expression that the node uses to determine if a user is in this namespace.
             */
            regex: string;
        }[];
        ...
        aliases: {
             ...
             * that no other user on the node may register aliases that match this namespace.
             */
            exclusive: boolean;

            /**
             * The regular expression that the node uses to determine if an alias is in this namespace.
             */
            regex: string;
        }[];
    };
    ...
    /**
     * If the application service is rate limited by the node. Optional.
     */
     ...
     * **Experimental**
     *
     * Should the application service receive ephemeral events from the node. Optional.
     */
...
export interface IAppserviceOptions {
    /**
     * The port to listen for requests from the node on.
     */
    port: number;
    ...
    /**
     * The URL to the node's client server API
     */
    nodeUrl: string;
...
export class Appservice extends EventEmitter {
     ...
     * from this appservice instance as well as any intents/SDNClients created
     * by the appservice.
     */
    ...
    private readonly bridgeInstance = new SDNBridge(this);
    ...
    constructor(private options: IAppserviceOptions) {
        ...
        this.app.get("/_api/app/v1/users/:userId", this.onUser.bind(this));
        this.app.get("/_api/app/v1/rooms/:roomAlias", this.onRoomAlias.bind(this));
        this.app.put("/_api/app/v1/transactions/:txnId", this.onTransaction.bind(this));
        this.app.get("/_api/app/v1/thirdparty/protocol/:protocol", this.onThirdpartyProtocol.bind(this));
        this.app.get("/_api/app/v1/thirdparty/user/:protocol", this.onThirdpartyUser.bind(this));
        this.app.get("/_api/app/v1/thirdparty/user", this.onThirdpartyUser.bind(this));
        this.app.get("/_api/app/v1/thirdparty/location/:protocol", this.onThirdpartyLocation.bind(this));
        this.app.get("/_api/app/v1/thirdparty/location", this.onThirdpartyLocation.bind(this));
        this.app.post("/_api/app/unstable/org.sdn.msc3983/keys/claim", this.onKeysClaim.bind(this));
        this.app.post("/_api/app/unstable/org.sdn.msc3984/keys/query", this.onKeysQuery.bind(this));

        this.app.post("/_api/app/v1/unstable/org.sdn.msc3983/keys/claim", this.onKeysClaim.bind(this));
        this.app.post("/unstable/org.sdn.msc3983/keys/claim", this.onKeysClaim.bind(this));
        this.app.post("/_api/app/v1/unstable/org.sdn.msc3984/keys/query", this.onKeysQuery.bind(this));
        this.app.post("/unstable/org.sdn.msc3984/keys/query", this.onKeysQuery.bind(this));
    ...
    /**
     * Get the application service's "bot" SDNClient (the sender_localpart).
     * Normally the botIntent should be used to ensure that the bot user is safely
     * handled.
     * @returns {SDNClient} The client for the application service itself.
     */
    public get botClient(): SDNClient {
    ...
    public getUserId(localpart: string): string {
        return `@${localpart}:${this.options.nodeName}`;
    }
    ...
    public getUserIdForSuffix(suffix: string): string {
        ...
        return `${this.userPrefix}${suffix}:${this.options.nodeName}`;
    }
    ...
    public getSuffixForUserId(userId: string): string {
        ...
        if (!userId || !userId.startsWith(this.userPrefix) || !userId.endsWith(`:${this.options.nodeName}`)) {
            // Invalid ID
            return null;
        }
        ...
        return userId
            ...
            .reverse()
            .slice(this.options.nodeName.length + 1)
            .reverse()
            .join('');
    }
    ...
    public getAliasForSuffix(suffix: string): string {
        ...
        return `${this.aliasPrefix}${suffix}:${this.options.nodeName}`;
    }
    ...
    public isNamespacedAlias(alias: string): boolean {
        ...
        return alias.startsWith(this.aliasPrefix) && alias.endsWith(":" + this.options.nodeName);
    }
    ...
    public setRoomDirectoryVisibility(networkId: string, roomId: string, visibility: "public" | "private") {
        ...
        return this.botClient.doRequest("PUT", `/_api/client/v3/directory/list/appservice/${networkId}/${roomId}`, null, {
            visibility,
        });
    }
    ...
    private async onTransaction(req: express.Request, res: express.Response): Promise<any> {
        ...
        this.pendingTransactions[txnId] = new Promise<void>(async (resolve) => {
            ...
            const deviceLists: { changed: string[], removed: string[] } = req.body["org.sdn.msc3202.device_lists"] ?? {
                changed: [],
                removed: [],
            };
            ...
            let otks = req.body["org.sdn.msc3202.device_one_time_keys_count"];
            const otks2 = req.body["org.sdn.msc3202.device_one_time_key_counts"];
            if (otks2 && !otks) {
                LogService.warn(
                    "Appservice",
                    "Your node is using an outdated field (device_one_time_key_counts) to talk to this appservice. "
                );
                otks = otks2;
            }
            if (otks) {
            ...
            const fallbacks = req.body["org.sdn.msc3202.device_unused_fallback_key_types"];
            if (fallbacks) {
    ...
    private handleThirdpartyObject(req: express.Request, res: express.Response, objType: string, sdnId?: string) {
        if (!this.isAuthed(req)) {
            res.status(401).json({ errcode: "AUTH_FAILED", error: "Authentication failed" });
            return;
        }
        ...
        if (protocol) { // If protocol is given, we are looking up a objects based on fields
            ...
            return;
        } else if (sdnId) { // If a user ID is given, we are looking up a remote objects based on a id
            this.emit(`thirdparty.${objType}.sdn`, sdnId, responseFunc);
            return;
        }
