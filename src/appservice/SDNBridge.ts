 ...
 * @category Application services
 */
export class SDNBridge {
    constructor(private appservice: Appservice) {
    }

    /**
     * Gets information about a remote user.
     * @param {Intent} userIntent The SDN user intent to get information on.
     ...
     * @param {Intent} userIntent The SDN user intent to store information on.
     ...
     * @param {string} sdnRoomId The SDN room ID to get information on.
     * @returns {Promise<IRemoteRoomInfo>} Resolves to the remote room information.
     */
    public async getRemoteRoomInfo<T extends IRemoteRoomInfo>(sdnRoomId: string): Promise<T> {
        ...
        return <Promise<T>>bridgeBot.underlyingClient.getRoomAccountData(REMOTE_ROOM_INFO_ACCOUNT_DATA_EVENT_TYPE, sdnRoomId);
    }

    /**
     * Sets information about a remote room. Calling this function will map the
     * provided remote room ID to the sdn room ID.
     * @param {string} sdnRoomId The SDN room ID to store information on.
     * @param {IRemoteRoomInfo} remoteInfo The remote room information to store
     * @returns {Promise<any>} Resolves when the information has been updated.
     */
    public async setRemoteRoomInfo<T extends IRemoteRoomInfo>(sdnRoomId: string, remoteInfo: T): Promise<any> {
        const bridgeBot = this.appservice.botIntent;
        await bridgeBot.ensureRegistered();
        // We do not need to ensure the user is joined to the room because we can associate
        // room account data with any arbitrary room.
        await bridgeBot.underlyingClient.setRoomAccountData(REMOTE_ROOM_INFO_ACCOUNT_DATA_EVENT_TYPE, sdnRoomId, remoteInfo);
        await this.updateRemoteRoomMapping(sdnRoomId, remoteInfo.id);
    }

    /**
     * Gets the SDN room ID for the provided remote room ID.
     * @param {string} remoteRoomId The remote room ID to look up.
     * @returns {Promise<string>} Resolves to the SDN room ID.
     */
    public async getsdnRoomIdForRemote(remoteRoomId: string): Promise<string> {
    ...
    /**
     * Gets a SDN user intent for the provided remote user ID.
     * @param {string} remoteUserId The remote user ID to look up.
     * @returns {Promise<Intent>} Resolves to the SDN user intent.
     */
    ...
    private async updateRemoteUserMapping(sdnUserId: string, remoteUserId: string): Promise<any> {
        const eventType = `${REMOTE_USER_MAP_ACCOUNT_DATA_EVENT_TYPE_PREFIX}.${remoteUserId}`;
        const bridgeBot = this.appservice.botIntent;
        await bridgeBot.ensureRegistered();
        await bridgeBot.underlyingClient.setAccountData(eventType, { id: sdnUserId });
    }

    private async updateRemoteRoomMapping(sdnRoomId: string, remoteRoomId: string): Promise<any> {
        const eventType = `${REMOTE_ROOM_MAP_ACCOUNT_DATA_EVENT_TYPE_PREFIX}.${remoteRoomId}`;
        const bridgeBot = this.appservice.botIntent;
        await bridgeBot.ensureRegistered();
        await bridgeBot.underlyingClient.setAccountData(eventType, { id: sdnRoomId });
    }
}
