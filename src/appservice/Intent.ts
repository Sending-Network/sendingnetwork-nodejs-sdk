 ...
 * in rooms to ensure the action being performed is possible. An Intent will ensure that the
...
export class Intent {
    ...
    private makeClient(withCrypto: boolean, accessToken?: string) {
        ...
        this.client = new SDNClient(this.options.nodeUrl, accessToken ?? this.options.registration.as_token, storage, cryptoStore);
    ...
    /**
     * Gets the underlying SDNClient that powers this Intent.
     */
    public get underlyingClient(): SDNClient {
        return this.client;
    }
                ...
                try {
                    ...
                    if (!prepared) {
                        ...
                        if (!accessToken) {
                            ...
                            const res = await this.client.doRequest("POST", "/_api/client/v3/login", {}, loginBody);
    ...
    public async ensureRegistered() {
        if (!(await Promise.resolve(this.storage.isUserRegistered(this.userId)))) {
            try {
                const result = await this.client.doRequest("POST", "/_api/client/v3/register", null, {
                    type: "m.login.application_service",
                ...
                if (err instanceof SDNError && err.errcode === "M_USER_IN_USE") {
