...
import { SDNError } from "./models/SDNError";
...
export async function doHttpRequest(
    ...
    LogService.debug("SDNHttpClient", "(REQ-" + requestId + ")", method + " " + url);

    // Don't log the request unless we're in debug mode. It can be large.
    if (LogService.level.includes(LogLevel.TRACE)) {
        if (qs) LogService.trace("SDNHttpClient", "(REQ-" + requestId + ")", "qs = " + JSON.stringify(qs));
        if (body && !Buffer.isBuffer(body)) LogService.trace("SDNHttpClient", "(REQ-" + requestId + ")", "body = " + JSON.stringify(redactObjectForLogging(body)));
        if (body && Buffer.isBuffer(body)) LogService.trace("SDNHttpClient", "(REQ-" + requestId + ")", "body = <Buffer>");
    }
    ...
    const { response, resBody } = await new Promise<{ response: any, resBody: any }>((resolve, reject) => {
        getRequestFn()(params, (err, res, rBody) => {
            if (err) {
                LogService.error("SDNHttpClient", "(REQ-" + requestId + ")", err);
                reject(err);
                return;
            }
    ...
    if (typeof (errBody) === "object" && 'errcode' in errBody) {
        ...
        LogService.error("SDNHttpClient", "(REQ-" + requestId + ")", redactedBody);
        throw new SDNError(errBody, response.statusCode);
    }
    ...
    if (LogService.level.includes(LogLevel.TRACE)) {
        ...
        LogService.trace("SDNHttpClient", "(REQ-" + requestId + " RESP-H" + response.statusCode + ")", redactedBody);
    }
