...
import { timedSDNClientFunctionCall } from "./metrics/decorators";
...
/**
 * A client that is capable of interacting with a sdn node.
 */
export class SDNClient extends EventEmitter {
    /**
    ...
    /**
     * Creates a new sdn client
     * @param {string} nodeUrl The node's client-server API URL
     * @param {string} accessToken The access token for the node
    ...
    constructor(
        ...
        if (this.nodeUrl.endsWith("/")) {
            this.nodeUrl = this.nodeUrl.substring(0, this.nodeUrl.length - 1);
        }
    ...
    public async getServerVersions(): Promise<ServerVersions> {
        if (!this.cachedVersions || (Date.now() - this.versionsLastFetched) >= VERSIONS_CACHE_MS) {
            this.cachedVersions = await this.doRequest("GET", "/_api/client/versions");
    ...
    /**
     * Retrieves an OpenID Connect token from the node for the current user.
    ...
    public async getOpenIDConnectToken(): Promise<OpenIDConnectToken> {
        ...
        return this.doRequest("POST", "/_api/client/v3/user/" + userId + "/openid/request_token", null, {});
    }
    ...
    public async getAccountData<T>(eventType: string): Promise<T> {
        ...
        return this.doRequest("GET", "/_api/client/v3/user/" + userId + "/account_data/" + eventType);
    }
    ...
    public async getRoomAccountData<T>(eventType: string, roomId: string): Promise<T> {
        ...
        return this.doRequest("GET", "/_api/client/v3/user/" + userId + "/rooms/" + roomId + "/account_data/" + eventType);
    }
    ...
    public async getSafeAccountData<T>(eventType: string, defaultContent: T = null): Promise<T> {
        try {
            ...
            LogService.warn("SDNClient", `Error getting ${eventType} account data:`, extractRequestError(e));
    ...
    public async getSafeRoomAccountData<T>(eventType: string, roomId: string, defaultContent: T = null): Promise<T> {
        try {
            ...
            LogService.warn("SDNClient", `Error getting ${eventType} room account data in ${roomId}:`, extractRequestError(e));
            return defaultContent;
        }
    }
    ...
    public async setAccountData(eventType: string, content: any): Promise<any> {
        const userId = encodeURIComponent(await this.getUserId());
        eventType = encodeURIComponent(eventType);
        return this.doRequest("PUT", "/_api/client/v3/user/" + userId + "/account_data/" + eventType, null, content);
    }
    ...
    public async setRoomAccountData(eventType: string, roomId: string, content: any): Promise<any> {
        ...
        eventType = encodeURIComponent(eventType);
        roomId = encodeURIComponent(roomId);
        return this.doRequest("PUT", "/_api/client/v3/user/" + userId + "/rooms/" + roomId + "/account_data/" + eventType, null, content);
    }
    ...
    public async getPresenceStatusFor(userId: string): Promise<Presence> {
        return this.doRequest("GET", "/_api/client/v3/presence/" + encodeURIComponent(userId) + "/status").then(r => new Presence(r));
    }
    ...
    public async setPresenceStatus(presence: PresenceState, statusMessage: string | undefined = undefined): Promise<any> {
        return this.doRequest("PUT", "/_api/client/v3/presence/" + encodeURIComponent(await this.getUserId()) + "/status", null, {
            presence: presence,
    ...
    public createRoomAlias(alias: string, roomId: string): Promise<any> {
        ...
        return this.doRequest("PUT", "/_api/client/v3/directory/room/" + alias, null, {
            "room_id": roomId,
        });
    }
    ...
    public deleteRoomAlias(alias: string): Promise<any> {
        alias = encodeURIComponent(alias);
        return this.doRequest("DELETE", "/_api/client/v3/directory/room/" + alias);
    }
    ...
    public setDirectoryVisibility(roomId: string, visibility: "public" | "private"): Promise<any> {
        roomId = encodeURIComponent(roomId);
        return this.doRequest("PUT", "/_api/client/v3/directory/list/room/" + roomId, null, {
    ...
    public getDirectoryVisibility(roomId: string): Promise<"public" | "private"> {
        roomId = encodeURIComponent(roomId);
        return this.doRequest("GET", "/_api/client/v3/directory/list/room/" + roomId).then(response => {
    ...
    public lookupRoomAlias(roomAlias: string): Promise<RoomDirectoryLookupResponse> {
        return this.doRequest("GET", "/_api/client/v3/directory/room/" + encodeURIComponent(roomAlias)).then(response => {
            return {
    ...
    public inviteUser(userId, roomId) {
        return this.doRequest("POST", "/_api/client/v3/rooms/" + encodeURIComponent(roomId) + "/invite", null, {
            user_id: userId,
        });
    }
    ...
    public kickUser(userId, roomId, reason = null) {
        return this.doRequest("POST", "/_api/client/v3/rooms/" + encodeURIComponent(roomId) + "/kick", null, {
            user_id: userId,
            reason: reason,
        });
    }
    ...
    public banUser(userId, roomId, reason = null) {
        return this.doRequest("POST", "/_api/client/v3/rooms/" + encodeURIComponent(roomId) + "/ban", null, {
            user_id: userId,
            reason: reason,
        });
    }
    ...
    public unbanUser(userId, roomId) {
        return this.doRequest("POST", "/_api/client/v3/rooms/" + encodeURIComponent(roomId) + "/unban", null, {
            user_id: userId,
        });
    }
    ...
    public async getWhoAmI(): Promise<IWhoAmI> {
        const whoami = await this.doRequest("GET", "/_api/client/v3/account/whoami");
    ...
    public async start(filter: any = null): Promise<any> {
        ...
        if (!filter || typeof (filter) !== "object") {
            LogService.trace("SDNClientLite", "No filter given or invalid object - using defaults.");
            filter = null;
        }

        LogService.trace("SDNClientLite", "Populating joined rooms to avoid excessive join emits");
        this.lastJoinedRoomIds = await this.getJoinedRooms();

        const userId = await this.getUserId();

        if (this.crypto) {
            LogService.debug("SDNClientLite", "Preparing end-to-end encryption");
            await this.crypto.prepare(this.lastJoinedRoomIds);
            LogService.info("SDNClientLite", "End-to-end encryption enabled");
        }
        ...
        if (existingFilter) {
            LogService.trace("SDNClientLite", "Found existing filter. Checking consistency with given filter");
            if (JSON.stringify(existingFilter.filter) === JSON.stringify(filter)) {
                LogService.trace("SDNClientLite", "Filters match");
        ...
        if (createFilter && filter) {
            LogService.trace("SDNClientLite", "Creating new filter");
            await this.doRequest("POST", "/_api/client/v3/user/" + encodeURIComponent(userId) + "/filter", null, filter).then(async response => {
        ...
        LogService.trace("SDNClientLite", "Starting sync with filter ID " + this.filterId);
    ...
    protected async startSync(emitFn: (emitEventType: string, ...payload: any[]) => Promise<any> = null) {
        ...
        const promiseWhile = async () => {
            if (this.stopSyncing) {
                LogService.info("SDNClientLite", "Client stop requested - stopping sync");
                return;
            }
            ...
            try {
                ...
                LogService.debug("SDNClientLite", "Received sync. Next token: " + token);
                ...
                LogService.error("SDNClientLite", "Error handling sync " + extractRequestError(e));
                const backoffTime = SYNC_BACKOFF_MIN_MS + Math.random() * (SYNC_BACKOFF_MAX_MS - SYNC_BACKOFF_MIN_MS);
                LogService.info("SDNClientLite", `Backing off for ${backoffTime}ms`);
    ...
    protected doSync(token: string): Promise<any> {
        LogService.debug("SDNClientLite", "Performing sync with token " + token);
        const conf = {
            full_state: false,
        ...
        return this.doRequest("GET", "/_api/client/v3/sync", conf, null, (token ? 40000 : 600000));
    }
    ...
    protected async processSync(raw: any, emitFn: (emitEventType: string, ...payload: any[]) => Promise<any> = null): Promise<any> {
        ...
        if (this.crypto) {
            ...
            if (raw['org.sdn.msc2732.device_unused_fallback_key_types']) {
                unusedFallbacks = raw['org.sdn.msc2732.device_unused_fallback_key_types'];
        ...
        for (const roomId in leftRooms) {
            ...
            if (!leaveEvent) {
                LogService.warn("SDNClientLite", "Left room " + roomId + " without receiving an event");
                continue;
            }
        ...
        for (const roomId in inviteRooms) {
            ...
            if (!inviteEvent) {
                LogService.warn("SDNClientLite", "Invited to room " + roomId + " without receiving an event");
                continue;
            }
            ...
            for (let event of room['timeline']['events']) {
                ...
                if (event['type'] === 'm.room.encrypted' && await this.crypto?.isRoomEncrypted(roomId)) {
                    ...
                    try {
                        ...
                        LogService.error("SDNClientLite", `Decryption error on ${roomId} ${event['event_id']}`, e);
    ...
    public getRawEvent(roomId: string, eventId: string): Promise<any> {
        return this.doRequest("GET", "/_api/client/v3/rooms/" + encodeURIComponent(roomId) + "/event/" + encodeURIComponent(eventId))
    ...
    public getRoomState(roomId: string): Promise<any[]> {
        return this.doRequest("GET", "/_api/client/v3/rooms/" + encodeURIComponent(roomId) + "/state")
    ...
    public getRoomStateEvent(roomId, type, stateKey): Promise<any> {
        const path = "/_api/client/v3/rooms/"
    ...
    public async getEventContext(roomId: string, eventId: string, limit = 10): Promise<EventContext> {
        const res = await this.doRequest("GET", "/_api/client/v3/rooms/" + encodeURIComponent(roomId) + "/context/" + encodeURIComponent(eventId), { limit });
        return {
    ...
    public getUserProfile(userId: string): Promise<any> {
        return this.doRequest("GET", "/_api/client/v3/profile/" + encodeURIComponent(userId));
    }
    ...
    public async setDisplayName(displayName: string): Promise<any> {
        const userId = encodeURIComponent(await this.getUserId());
        return this.doRequest("PUT", "/_api/client/v3/profile/" + userId + "/displayname", null, {
     ...
     * @param {string} avatarUrl the new avatar URL for the user, in the form of a SDN Content URI
     * @returns {Promise<any>} resolves when complete
     */
    @timedSDNClientFunctionCall()
    public async setAvatarUrl(avatarUrl: string): Promise<any> {
        const userId = encodeURIComponent(await this.getUserId());
        return this.doRequest("PUT", "/_api/client/v3/profile/" + userId + "/avatar_url", null, {
    ...
    public async joinRoom(roomIdOrAlias: string, viaServers: string[] = []): Promise<string> {
        const apiCall = (targetIdOrAlias: string) => {
            ...
            return this.doRequest("POST", "/_api/client/v3/join/" + targetIdOrAlias, qs, {}).then(response => {
    ...
    public getJoinedRooms(): Promise<string[]> {
        return this.doRequest("GET", "/_api/client/v3/joined_rooms").then(response => response['joined_rooms']);
    }
    ...
    public getJoinedRoomMembers(roomId: string): Promise<string[]> {
        return this.doRequest("GET", "/_api/client/v3/rooms/" + encodeURIComponent(roomId) + "/joined_members").then(response => {
    ...
    public async getJoinedRoomMembersWithProfiles(roomId: string): Promise<{ [userId: string]: { display_name?: string, avatar_url?: string } }> {
        return (await this.doRequest("GET", "/_api/client/v3/rooms/" + encodeURIComponent(roomId) + "/joined_members")).joined;
    }
    ...
    private getRoomMembersAt(roomId: string, membership: Membership | null, notMembership: Membership | null, atToken: string | null): Promise<MembershipEvent[]> {
        ...
        return this.doRequest("GET", "/_api/client/v3/rooms/" + encodeURIComponent(roomId) + "/members", qs).then(r => {
    ...
    public leaveRoom(roomId: string, reason?: string): Promise<any> {
        return this.doRequest("POST", "/_api/client/v3/rooms/" + encodeURIComponent(roomId) + "/leave", null, { reason });
    }
    ...
    public forgetRoom(roomId: string): Promise<{}> {
        return this.doRequest("POST", "/_api/client/v3/rooms/" + encodeURIComponent(roomId) + "/forget");
    }
    ...
    public sendReadReceipt(roomId: string, eventId: string): Promise<any> {
        return this.doRequest("POST", "/_api/client/v3/rooms/" + encodeURIComponent(roomId) + "/receipt/m.read/" + encodeURIComponent(eventId), null, {});
    }
    ...
    public async setTyping(roomId: string, typing: boolean, timeout = 30000): Promise<any> {
        const userId = await this.getUserId();
        return this.doRequest("PUT", "/_api/client/v3/rooms/" + encodeURIComponent(roomId) + "/typing/" + encodeURIComponent(userId), null, {
            typing,
            timeout,
        });
    }
    ...
    public redactEvent(roomId: string, eventId: string, reason: string | null = null): Promise<string> {
        ...
        return this.doRequest("PUT", `/_api/client/v3/rooms/${encodeURIComponent(roomId)}/redact/${encodeURIComponent(eventId)}/${txnId}`, null, content).then(response => {
            return response['event_id'];
        });
    }
    ...
    public createRoom(properties: RoomCreateOptions = {}): Promise<string> {
        return this.doRequest("POST", "/_api/client/v3/createRoom", null, properties).then(response => {
            return response['room_id'];
        });
    }
    ...
    public mxcToHttp(mxc: string): string {
        ...
        const originNode = parts[0];
        const mediaId = parts.slice(1, parts.length).join('/');
        return `${this.nodeUrl}/_api/media/v3/download/${encodeURIComponent(originNode)}/${encodeURIComponent(mediaId)}`;
    }
    ...
    public mxcToHttpThumbnail(mxc: string, width: number, height: number, method: "crop" | "scale"): string {
        ...
        return downloadUri.replace("/_api/media/v3/download", "/_api/media/v3/thumbnail")
            + `?width=${width}&height=${height}&method=${encodeURIComponent(method)}`;
    }

    /**
     * Uploads data to the node's media repository. Note that this will <b>not</b> automatically encrypt
    ...
    public uploadContent(data: Buffer, contentType = "application/octet-stream", filename: string = null): Promise<string> {
        ...
        return this.doRequest("POST", "/_api/media/v3/upload", { filename: filename }, data, 60000, false, contentType)
            .then(response => response["content_uri"]);
    }

    /**
     * Download content from the node's media repository. Note that this will <b>not</b> automatically decrypt
    ...
    public async downloadContent(mxcUrl: string, allowRemote = true): Promise<{ data: Buffer, contentType: string }> {
        ...
        const path = `/_api/media/v3/download/${domain}/${mediaId}`;
    ...
    /**
     * Uploads data to the node's media repository after downloading it from the
     * provided URL.
    ...
    public uploadContentFromUrl(url: string): Promise<string> {
        return new Promise<{ body: Buffer, contentType: string }>((resolve, reject) => {
            ...
            getRequestFn()(params, (err, response, resBody) => {
                if (err) {
                    LogService.error("SDNClientLite", "(REQ-" + requestId + ")", extractRequestError(err));
                    reject(err);
                } else {
                    const contentType = response.headers['content-type'] || "application/octet-stream";

                    LogService.trace("SDNClientLite", "(REQ-" + requestId + " RESP-H" + response.statusCode + ")", "<data>");
                    if (response.statusCode < 200 || response.statusCode >= 300) {
                        LogService.error("SDNClientLite", "(REQ-" + requestId + ")", "<data>");
                        reject(response);
    ...
    /**
     * Gets a Space.
     * This API does not work with unstable spaces (e.g. org.sdn.msc.1772.space)
     *
    ...
    public async uploadDeviceOneTimeKeys(keys: OTKs): Promise<OTKCounts> {
        return this.doRequest("POST", "/_api/client/v3/keys/upload", null, {
            one_time_keys: keys,
    ...
    public async uploadFallbackKey(fallbackKey: FallbackKey): Promise<OTKCounts> {
        ...
        return this.doRequest("POST", "/_api/client/v3/keys/upload", null, {
            "org.sdn.msc2732.fallback_keys": keyObj,
    ...
    public async getUserDevices(userIds: string[], federationTimeoutMs = 10000): Promise<MultiUserDeviceListResponse> {
        ...
        return this.doRequest("POST", "/_api/client/v3/keys/query", {}, {
    ...
    public async getOwnDevices(): Promise<OwnUserDevice[]> {
        return this.doRequest("GET", "/_api/client/v3/devices").then(r => {
            return r['devices'];
        });
    }
    ...
    public async claimOneTimeKeys(userDeviceMap: Record<string, Record<string, OTKAlgorithm>>, federationTimeoutMs = 10000): Promise<OTKClaimResponse> {
        return this.doRequest("POST", "/_api/client/v3/keys/claim", {}, {
            timeout: federationTimeoutMs,
    ...
    public async sendToDevices(type: string, messages: Record<string, Record<string, any>>): Promise<void> {
        ...
        return this.doRequest("PUT", `/_api/client/v3/sendToDevice/${encodeURIComponent(type)}/${encodeURIComponent(txnId)}`, null, {
            messages: messages,
        });
    }
    ...
    public async getRelationsForEvent(roomId: string, eventId: string, relationType?: string, eventType?: string): Promise<{ chunk: any[] }> {
        let url = `/_api/client/v1/rooms/${encodeURIComponent(roomId)}/relations/${encodeURIComponent(eventId)}`;
        if (relationType) {
            url += `/${relationType}`;
        }
        if (eventType) {
    ...
    /**
     * Performs a web request to the node, applying appropriate authorization headers for
     * this client.
     * @param {"GET"|"POST"|"PUT"|"DELETE"} method The HTTP method to use in the request
     * @param {string} endpoint The endpoint to call. For example: "/_api/client/v3/account/whoami"
    ...
    public doRequest(method, endpoint, qs = null, body = null, timeout = 60000, raw = false, contentType = "application/json", noEncoding = false): Promise<any> {
        ...
        if (this.impersonatedDeviceId) {
            if (!qs) qs = { "org.sdn.msc3202.device_id": this.impersonatedDeviceId };
            else qs["org.sdn.msc3202.device_id"] = this.impersonatedDeviceId;
        }
        const headers = {};
        if (this.accessToken) {
            headers["Authorization"] = `Bearer ${this.accessToken}`;
        }
        return doHttpRequest(this.nodeUrl, method, endpoint, qs, body, headers, timeout, raw, contentType, noEncoding);
    }
}
