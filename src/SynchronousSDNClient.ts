...
/**
 * A SDNClient class that handles events in sync for the /sync loop, instead
 ...
 * of doing nothing.
 */
export abstract class SynchronousSDNClient extends SDNClient {
    /**
     * Creates a new SynchronousSDNClient. Note that this accepts a SDNClient, though
     * much of the class's properties are not brought over. Always convert your SDNClient
     * instance to a SynchronousSDNClient as soon as possible to avoid diversion in which
     * properties are proxied over.
     * @param {SDNClient} baseClient The client to wrap.
     */
    protected constructor(baseClient: SDNClient) {
        super(baseClient.nodeUrl, baseClient.accessToken, baseClient.storageProvider);
    }
