...
export class Permalinks {
    ...
    public static forRoom(roomIdOrAlias: string, viaServers: string[] = []): string {
        return `https://sdn.to/#/${roomIdOrAlias}${Permalinks.encodeViaArgs(viaServers)}`;
    }
    ...
    public static forEvent(roomIdOrAlias: string, eventId: string, viaServers: string[] = []): string {
        return `https://sdn.to/#/${roomIdOrAlias}/${eventId}${Permalinks.encodeViaArgs(viaServers)}`;
    }

    /**
     * Parses a permalink URL into usable parts.
     * @param {string} sdnTo The sdn.to URL to parse.
     * @returns {PermalinkParts} The parts of the permalink.
     */
    public static parseUrl(sdnTo: string): PermalinkParts {
        const sdnToRegexp = /^https:\/\/sdn\.to\/#\/(?<entity>[^/?]+)\/?(?<eventId>[^?]+)?(?<query>\?[^]*)?$/;

        const url = sdnToRegexp.exec(sdnTo)?.groups;
        if (!url) {
            throw new Error("Not a valid sdn.to URL");
        }
