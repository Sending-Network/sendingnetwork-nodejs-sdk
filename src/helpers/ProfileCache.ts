...
import { extractRequestError, LogService, SDNClient, SDNProfile } from "..";
...
export class ProfileCache {
    private cache: LRU<CacheKey, SDNProfile>;
     ...
     * @param {SDNClient} client The client to use to get profile updates.
     */
    constructor(maxEntries: number, maxAgeMs: number, private client: SDNClient) {
        this.cache = new LRU({
            max: maxEntries,
            ttl: maxAgeMs,
        });
     ...
     * @param {SDNClient} client The client to watch for profile changes with.
     */
    public watchWithClient(client: SDNClient) {
     ...
     * SDNClient to use, or null to use the appservice's bot client. The same
     ...
     */
    public watchWithAppservice(appservice: Appservice, clientFn: (userId: string, roomId: string) => SDNClient = null) {
     ...
     * @returns {Promise<SDNProfile>} Resolves to the user's profile.
     */
    public async getUserProfile(userId: string, roomId: string = null): Promise<SDNProfile> {
        const cacheKey = this.getCacheKey(userId, roomId);
        const cached = this.cache.get(cacheKey);
        if (cached) return Promise.resolve(<SDNProfile>cached);
    ...
    private async getUserProfileWith(userId: string, roomId: string, client: SDNClient): Promise<SDNProfile> {
        try {
            if (roomId) {
                const membership = await client.getRoomStateEvent(roomId, "m.room.member", userId);
                return new SDNProfile(userId, membership);
            } else {
                const profile = await client.getUserProfile(userId);
                return new SDNProfile(userId, profile);
            }
        } catch (e) {
            ...
            return new SDNProfile(userId, {});
        }
    }

    private async tryUpdateProfile(roomId: string, memberEvent: MembershipEvent, client: SDNClient) {
        ...
        if (this.cache.has(roomCacheKey)) {
            this.cache.set(roomCacheKey, new SDNProfile(memberEvent.membershipFor, memberEvent.content));
        }
