...
import { SDNProfile, SDNProfileInfo } from "../models/SDNProfile";
import { IdentityServerAccount, IdentityServerInvite } from "../models/IdentityServerModels";

/**
 * A way to access an Identity Server using the Identity Service API from a SDNClient.
 * @category Identity Servers
 */
export class IdentityClient {
    /**
     * The metrics instance for this client. Note that metrics for the underlying SDNClient will
    ...
    private constructor(public readonly accessToken: string, public readonly serverUrl: string, public readonly SDNClient: SDNClient) {
    ...
    public getAccount(): Promise<IdentityServerAccount> {
        return this.doRequest("GET", "/_api/identity/v2/account");
    }
    ...
    public getTermsOfService(): Promise<Policies> {
        return this.doRequest("GET", "/_api/identity/v2/terms");
    }
    ...
    public acceptTerms(termsUrls: string[]): Promise<void> {
        return this.doRequest("POST", "/_api/identity/v2/terms", null, {
    ...
    public async lookup(identifiers: Threepid[], allowPlaintext = false): Promise<string[]> {
        const hashInfo = await this.doRequest("GET", "/_api/identity/v2/hash_details");
        ...
        const resp = await this.doRequest("POST", "/_api/identity/v2/lookup", null, body);
    ...
    public async makeEmailInvite(emailAddress: string, roomId: string): Promise<IdentityServerInvite> {
        const req = {
            ...
            medium: "email",
            room_id: roomId,
            sender: await this.SDNClient.getUserId(),
        };

        const tryFetch = async (eventType: string, stateKey: string): Promise<any> => {
            try {
                return await this.SDNClient.getRoomStateEvent(roomId, eventType, stateKey);
            } catch (e) {
                return null;
            }
        };
        ...
        let profileInfo: SDNProfileInfo;
        try {
            profileInfo = await this.SDNClient.getUserProfile(await this.SDNClient.getUserId());
        } catch (e) {
            // ignore
        }
        const senderProfile = new SDNProfile(await this.SDNClient.getUserId(), profileInfo);
        ...
        return await this.doRequest("POST", "/_api/identity/v2/store-invite", qs, inviteReq);
    }
     ...
     * @param {string} endpoint The endpoint to call. For example: "/_api/identity/v2/account"
     ...
     */
    public static async acquire(oidc: OpenIDConnectToken, serverUrl: string, mxClient: SDNClient): Promise<IdentityClient> {
        const account = await doHttpRequest(serverUrl, "POST", "/_api/identity/v2/account/register", null, oidc);
